...
public class SubscriptionManager {
    ...
    private static JPanel createAddSubscriptionPanel() {
        ...
        // Purchase date: automatically use today's date by default, or allow
        // the user to enter a custom date in YYYY-MM-DD format.
        JCheckBox autoPurchaseDateBox = new JCheckBox("Use current date automatically", true);
        autoPurchaseDateBox.setOpaque(false);
        JTextField purchaseDateField = new JTextField(LocalDate.now().format(DATE_FMT));
        purchaseDateField.setEnabled(false);

        autoPurchaseDateBox.addActionListener(e -> {
            boolean automatic = autoPurchaseDateBox.isSelected();
            purchaseDateField.setEnabled(!automatic);
            if (automatic) {
                purchaseDateField.setText(LocalDate.now().format(DATE_FMT));
            }
        });
        ...
        form.add(new JLabel("Purchase Date:"));
        JPanel purchaseDatePanel = new JPanel(new BorderLayout(8, 0));
        purchaseDatePanel.setOpaque(false);
        purchaseDatePanel.add(autoPurchaseDateBox, BorderLayout.WEST);
        purchaseDatePanel.add(purchaseDateField, BorderLayout.CENTER);
        form.add(purchaseDatePanel);
        ...
        saveBtn.addActionListener(e -> {
            try {
                ...
                if (autoPurchaseDateBox.isSelected()) {
                    purchaseLocalDate = LocalDate.now();
                } else {
                    String customDate = purchaseDateField.getText().trim();
                    try {
                        purchaseLocalDate = LocalDate.parse(customDate, DATE_FMT);
                    } catch (DateTimeParseException ex) {
                        throw new IllegalArgumentException(
                                "Invalid purchase date. Please use YYYY-MM-DD format.");
                    }

                    // Do not allow a future purchase date.
                ...
                String purchaseDate = purchaseLocalDate.format(DATE_FMT);
                String purchaseTime = LocalTime.now().format(TIME_FMT);

                // Renewal is calculated from the purchase date, not today's date.
                LocalDate renewalLocalDate = plan.equalsIgnoreCase("Monthly")
                        ? purchaseLocalDate.plusMonths(monthsRequested[0])
                        : purchaseLocalDate.plusYears(1);
                ...
                autoPurchaseDateBox.setSelected(true);
                purchaseDateField.setText(LocalDate.now().format(DATE_FMT));
                purchaseDateField.setEnabled(false);
